...
def soup_to_links(soup): 
	links = []
	[links.append(link.get('href')) for link in soup.find_all(r'a') if \
	re.search(r'https://www.thecdg.co.uk/members/', link.get('href')) \
...
def populate_null_fields(data): 
	for label in data:          
		if label == 'Nickname':
			...
			data[label].extend(['N/A' for i in range(len(data['Nickname']) -\
			                   len(data[label]))])
		else:
			continue
	return data
	...
	for lists in data:   
		...
		if '' in lists: # skip empty strings
			...
			if label in final and len(final[label]) == track: 
				...
				final[label].extend(['N/A' for i in range(track-1 -\
				                     len(final[label]))])
				final[label].append(row_value)
			else:
...
df = pd.concat([pd.DataFrame(v, columns=[k]) for k, v in next(final)\
.items()], axis=1) 
